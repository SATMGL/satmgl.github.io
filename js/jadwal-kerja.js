...
function updateKeterangan() {
  ...
  keterangan.textContent = unit
    ? `PELAKSANAAN KERJA UNIT ${unit.toUpperCase()} ${periode.toUpperCase()}`
...
function renderTable(data, days, year, month, unit) {
  ...
  if (thead) {
    ...
    thead.innerHTML = `
      <tr>
        ...
        <th colspan="${days}">PELAKSANAAN UNIT ${unit} BULAN ${monthLabel} ${year}</th>
      </tr>
      <tr>${hariHtml}</tr>
      <tr>${tglHtml}</tr>`;
  }
  ...
  const keterangan = document.querySelector('.keterangan');
  if (keterangan) {
    keterangan.textContent = `PELAKSANAAN KERJA UNIT ${unit}   ${monthLabel.toUpperCase()} ${year}`;
  }
...
function drawTableToCanvas(data, days) {
  ...
  ctx.fillText(`PELAKSANAAN UNIT ${unit} BULAN ${monthLabel} ${year}`, totalW / 2, 32);
  ...
  drawCell(startX + namaColW, startY, dayColW * days, rowH,
    '#1976d2', '#fff', `PELAKSANAAN UNIT ${unit}`, true);
