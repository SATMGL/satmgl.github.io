// =====================================================================
  ...
  // Nama lengkap maks 3 kata — kata ke-4 dst disingkat inisial
  function namaLengkap3(nama) {
    const parts = nama.trim().split(/\s+/);
    if (parts.length <= 3) return parts.join(' ');
    const depan3 = parts.slice(0, 3).join(' ');
    const singkatan = parts.slice(3).map(w => w.charAt(0).toUpperCase() + '.').join('');
    return depan3 + ' ' + singkatan;
  }
  ...
  function fillTable() {
    ...
    if (secEl) {
      secEl.innerHTML = '<span style="color:#004B87;font-weight:bold;font-size:6pt;font-family:Arial;white-space:normal;line-height:1.2;">' + namaLengkap3(namaVal) + '</span>';
    }
  ...
  async function generateAdminPages(mode) {
    ...
    for (let i = 0; i < daftarTerpilih.length; i++) {
      ...
      try {
        ...
        // Nama di bawah kolom SECURITY (lengkap 3 kata)
        const secEl = page.querySelector('#fill-security-nama');
        if (secEl) {
          secEl.innerHTML = '<span style="color:#004B87;font-weight:bold;font-size:6pt;font-family:Arial;white-space:normal;line-height:1.2;">' + namaLengkap3(nama) + '</span>';
        }
    ...
    if (mode === 'pdf') {
      ...
      // Render tiap halaman secara individual — hindari html2pdf memotong konten
      showLoading();
      try {
        // Lampirkan container ke DOM sementara agar html2canvas bisa render
        ...
        // Format px 794×1123 agar tidak ada konversi mm
        ...
        for (let i = 0; i < pages.length; i++) {
          ...
          // Inject style sementara agar html2canvas render border-collapse dengan benar
          const fixStyle = document.createElement('style');
          fixStyle.id = 'hc-border-fix';
          fixStyle.textContent = `
            /* Paksa border pada cell yang hanya punya bottom+right agar tidak hilang saat rendering */
            table.waffle td, table.waffle th {
              border-top: 1px solid transparent;
              border-left: 1px solid transparent;
            }
            /* Baris header tabel — tambah border-top eksplisit */
            table.waffle tr:first-child td,
            table.waffle tr:nth-child(2) td,
            table.waffle tr:nth-child(3) td {
              border-top: 2px solid #000 !important;
            }
            /* Kolom pertama — tambah border-left eksplisit */
            table.waffle td:first-child {
              border-left: 2px solid #000 !important;
            }
            /* Sel rowspan besar di area ttd — tambah semua border */
            .s27 {
            ...
            /* Baris terakhir tanda tangan */
            .s32, .s33, .s34 {
              border-top: 1px solid #000 !important;
            }
            /* Border-frame dan border-outer tetap solid */
            .border-outer { border: 2px solid #000 !important; }
            .border-frame  { border: 2px solid #000 !important; }
          `;
          page.appendChild(fixStyle);
          ...
          const canvas = await html2canvas(page, {
            ...
            windowWidth     : 794,
            onclone         : (doc) => {
              // Pastikan semua cell visible di clone
              doc.querySelectorAll('table.waffle td').forEach(td => {
          ...
          const fs = page.querySelector('#hc-border-fix');
          if (fs) fs.remove();
      ...
      win.document.write('<html><head><style>@page{margin:0;size:A4 portrait;}body{margin:0;padding:0;background:#fff;}.page{position:relative;width:794px;height:1123px;overflow:hidden;box-sizing:border-box;page-break-after:always;page-break-inside:avoid;}.page:last-child{page-break-after:avoid;}*{-webkit-print-color-adjust:exact;print-color-adjust:exact;}</style><link rel="stylesheet" href="../css/timesheet.css"></head><body>');
  ...
  // Intercept tombol back HP — hierarki: tutup modal/panel dulu, baru ke index
  history.pushState({ page: 'timesheet' }, '');
  window.addEventListener('popstate', () => {
    history.pushState({ page: 'timesheet' }, ''); // push lagi agar back berikutnya juga ter-intercept

    // 1. Modal edit terbuka?
    ...
    // 2. Modal admin (password) terbuka?
    ...
    // 3. Panel admin slide-in terbuka?
    if (typeof adminPanelOpen !== 'undefined' && adminPanelOpen) {
      toggleAdminPanel();
      return;
    }

    // 4. Tidak ada yang terbuka → kembali ke index
    window.location.href = '../index.html';
  });
  ...
  function openEditModal() {
    // Subtitle: NAMA — UNIT
    const nama  = (cddNama._getValue() || '').trim();
    const unit  = (cddUnit._getValue() || '').trim();
    const sub   = document.getElementById('edit-modal-subtitle');
    if (sub) sub.textContent = nama + (unit ? '  —  ' + unit : '');
    ...
    const selTgl = document.getElementById('edit-tgl-tgl');
    const selThn = document.getElementById('edit-tgl-thn');
    // Build tanggal 1-31
    selTgl.innerHTML = '<option value="">Tgl</option>';
    for (let d = 1; d <= 31; d++) selTgl.innerHTML += '<option>' + d + '</option>';
    // Build tahun (1990 - tahun sekarang)
    const tahunSkrg = new Date().getFullYear();
    selThn.innerHTML = '<option value="">Tahun</option>';
    for (let y = tahunSkrg; y >= 1990; y--) selThn.innerHTML += '<option>' + y + '</option>';

    // Parse nilai yang sudah ada, contoh: "1 SEPTEMBER 2019"
    const tglRaw = (document.getElementById('fill-tglmasuk').textContent || '').trim();
    const BULAN_LIST = ['JANUARI','FEBRUARI','MARET','APRIL','MEI','JUNI','JULI','AGUSTUS','SEPTEMBER','OKTOBER','NOVEMBER','DESEMBER'];
    if (tglRaw) {
      const parts = tglRaw.split(/\s+/);
      if (parts[0]) selTgl.value = parts[0];
      if (parts[1]) document.getElementById('edit-tgl-bln').value = parts[1].toUpperCase();
      if (parts[2]) selThn.value = parts[2];
    }
  ...
  function applyEditData() {
    ...
    const _tgl = document.getElementById('edit-tgl-tgl').value;
    const _bln = document.getElementById('edit-tgl-bln').value;
    const _thn = document.getElementById('edit-tgl-thn').value;
    const tglmasuk = (_tgl && _bln && _thn) ? _tgl + ' ' + _bln + ' ' + _thn : '';
    ...
    const lokasi = cddUnit._getValue()  || '';
    showLoading();
  ...
  // =====================================================================
  function cetakForm() {
    // Hapus sementara inline transform agar print pakai @media print CSS murni
    const page = document.querySelector('.page');
    const prevTransform   = page.style.transform;
    const prevMarginLeft  = page.style.marginLeft;
    const prevMarginTop   = page.style.marginTop;
    ...
    // Restore setelah dialog print ditutup
    setTimeout(() => {
      page.style.transform    = prevTransform;
      page.style.marginLeft   = prevMarginLeft;
      page.style.marginTop    = prevMarginTop;
