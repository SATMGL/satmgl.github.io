...
function _setJudulTabel(judul, sub) {
  const el = document.querySelector('.jadwal-judul');
  if (!el) return;
  if (sub) {
    el.innerHTML = `
      <span style="display:block; text-align:center; line-height:1.3;">${judul}</span>
      <span style="display:block; text-align:center; font-size:0.8em; font-weight:normal;
        opacity:0.85; line-height:1.3; letter-spacing:0.05em;">${sub}</span>`;
  } else {
    el.innerHTML = `<span style="display:block; text-align:center;">${judul}</span>`;
  }
}
...
async function init() {
  ...
  if (linkTema) linkTema.href = `../css/${MODE.toLowerCase()}.css`;
  ...
  setTimeout(() => { if (typeof _initBackIntercept === 'function') _initBackIntercept(); }, 0);
  ...
  try {
    ...
    if (cache && cache.bulan && cache.unit) {
      ...
      _styleBtn('btn-unit', true, selectedUnit);
      ...
      // Aktifkan tombol sesuai role dari cache
...
async function loadJadwal() {
  ...
  try {
    ...
    renderJadwal(data);
    if (typeof setZoomPreset === 'function') setZoomPreset(0.7);
...
function renderJadwal(data) {
  ...
  _setJudulTabel(
    JUDUL_MODE[MODE] || 'JADWAL KERJA',
    `${data.unitName}  ·  ${bulanLabel} ${selectedBulan.tahun}`);
...
function tutupJadwal() {
  ...
  _setJudulTabel(
    JUDUL_MODE[MODE] || 'JADWAL KERJA',
    selectedBulan ? `${BULAN_NAMA[selectedBulan.value]} ${selectedBulan.tahun}` : null);
  setBtnAktif('btn-download', false);
  setBtnAktif('btn-edit', false);
}
